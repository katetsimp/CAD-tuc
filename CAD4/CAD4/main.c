...
// Function prototypes
int addIO(Port *newPort, char name[MAX_CHARACTERS], char label[MAX_CHARACTERS], char IOnames[MAX_DEFINITION][MAX_CHARACTERS], int numofIOnames, int i, int c1);
int makePort(Iodeclaretion *new_iodec, int c1, Port *newPort);
...
int addIO(Port *newPort,char name[MAX_CHARACTERS], char label[MAX_CHARACTERS],char IOnames[MAX_DEFINITION][MAX_CHARACTERS],int numofIOnames,int i,int c1){
    ...
    c1=makePort(new_iodec,c1,newPort);
...
c1 = addIO(current_entity->PortMap, port_name, port_label, io_names, num_io_names, num_io_names, c1);
...
netlist* makeOutputFile(Entity_list*new_entity_list){
   int id=0; // unique identifier for each component
   char type[MAX_CHARACTERS]={"FULL_ADDER"}; // type of component
   char inputs[MAX_DEFINITION][MAX_CHARACTERS]={0}; // input ports of the component
   int numofinputs=0; // number of input ports
   int counter=0;
   int counter1=0;
   int c;
   int label;
...
new_Comp->numofinputs=new_entity_list->entity_array[i].variables_dec->ArrayOfVariables[n].value;
...
new_net->outputarray = new_out_array;
...
char*OutputFile ="full_adder_netlist";
new_entity_list=ReadInputFile(Filename,new_entity_list);
netlist*new_net = (netlist*)malloc(sizeof(netlist));

new_net=makeOutputFile(new_entity_list);
FILE* fp= fopen(OutputFile,"w");

//write in File
for (int i=0;i<new_net->new_Component_array->num_components;i++){


        fprintf(fp, "U%d %s ", new_net->new_Component_array->components[i].id, new_net->new_Component_array->components[i].type);
      for(int j=0;j<new_net->new_Component_array->components[i].numofinputs;j++){
            if (strcmp(new_net->new_Component_array->components[i].inputs[j], "") == 0) {
                break;
            }
            if (strcmp(new_net->new_Component_array->components[i].inputs[j+1], "") == 0){


            fprintf(fp,"%s", new_net->new_Component_array->components[i].inputs[j]);
    ...
    for (int k=0;k<new_net->outputarray->output_num;k++){


    fprintf(fp,"%s= U%d_%s\n", new_net->outputarray->Output_Dec_array[k].Output,new_net->outputarray->Output_Dec_array[k].origin,new_net->outputarray->Output_Dec_array[k].label);
